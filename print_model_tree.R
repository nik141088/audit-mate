...
tree = h2o.getModelTree(model = processed.gbm, tree_number = 1)

# tree = h2o.getModelTree(model = H2O_MODEL, tree_number = 1)




data_tree = createDataTree(tree)
...
GetFontName <- function(node) {switch(node$type, 
                                      split = 'Palatino-bold', 
                                      leaf = 'Palatino')}
SetEdgeStyle(data_tree, fontname = 'Palatino-italic', 
             label = GetEdgeLabel, labelfloat = TRUE,
             fontsize = "26", fontcolor='royalblue4')
SetNodeStyle(data_tree, fontname = GetFontName, shape = GetNodeShape, 
             fontsize = "26", fontcolor='royalblue4',
             height="0.75", width="1")

SetGraphStyle(data_tree, rankdir = "LR", dpi=70.)

plot(data_tree, output = "graph")
